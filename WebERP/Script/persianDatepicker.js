; (function () {
    $.fn.persianDatepicker = function (options, gEl) {
        var pluginName = 'persianDatepicker';
        var instance = this.data(pluginName);
        if (!instance) {
            return this.each(function () {
                return $(this).data(pluginName, new persianDatepicker(this, options, gEl));
            });
        }
    ...
    var persianDatepicker = (function () {
        function persianDatepicker(element, userOptions, gEl) {
            var defaults = {
            ...
            var self = this;
            self.el = $(element);
            self.gEl = $(document.getElementById(gEl));
            var el = self.el;
            var gEl = self.gEl;
            ...
            if (options.selectedBefore && !options.showGregorianDate) {
                if (self.options.selectedDate != undefined) {
                    self.showDate(el, self.persianDate.parse(self.options.selectedDate).toString(options.formatDate), 0);
                    self.showDate(gEl, self.persianDate.parse(self.options.selectedDate).gDate._toString(self.options.formatDate), 1);
                }
                else {
                    self.showDate(el, self.now().toString(options.formatDate), 0);
                    self.showDate(gEl, self.now().gDate._toString(self.options.formatDate), 1);
                }
            }

            if (options.selectedBefore && options.showGregorianDate) {
                if (self.options.selectedDate != undefined) {
                    self.showDate(el, self.persianDate.parse(self.options.selectedDate).gDate._toString(self.options.formatDate), 1);
                    //self.showDate(el, self.persianDate.parse(self.options.selectedDate).toString(options.formatDate));
                }
                else
                    self.showDate(el, self.now().gDate._toString(self.options.formatDate), 1);
            }
        ...
        persianDatepicker.prototype = {
                ...
                for (var row = 0, cellIndex = 0; row < 5 + 1; row++) {
                    ...
                    for (var col = 0; col < 7; col++, cellIndex++) {
                        if (cellIndex < _start || cellIndex - _start + 1 > _end) {
                            ...
                            _cell.data("date", { jDate: _dt.toString("YYYY/MM/DD"), gDate: self.jDateFunctions.getGDate(_dt)._toString(self.options.formatDate) });
                            _cell
                                    ...
                                    .bind("click", function () {
                                        ...
                                        if (self.options.showGregorianDate)
                                            ...
                                            self.el.attr("owner", "630N");
                                            self.el.attr("v63a0Nl", $(this).data('date').gDate);
                                            self.el.attr("value", $(this).data('date').jDate);
                                            //self.el.val($(this).data('date').jDate);
                                            //self.showDate(self.gEl, $(this).data('date').gDate);
                                        }
                                        self.calendar.hide();
                                    });
                        }
            ...
            footer: function () {
                ...
                if (self.options.selectableMonths._indexOf(self.persianDate.month) > -1) {
                    ...
                    _goToday.data("date", { jDate: self.now().toString("YYYY/MM/DD"), gDate: self.jDateFunctions.getGDate(self.now())._toString(self.options.formatDate) });
                    _goToday
            ...
            showDate: function (el, v, m) {
                var self = this;
                if (el.is('input')) {
                    if (m == 0)
                        el.val(self.persianDate.parse(v).toString(self.options.formatDate));
                    else
                        el.val(self.persianDate.parse(v).gDate.toString(self.options.formatDate));
                } else {
                    if (m == 0)
                        el.html(self.persianDate.parse(v).toString(self.options.formatDate));
                    else
                        el.html(self.persianDate.parse(v).gDate.toString(self.options.formatDate));

                }
                el.data('date', { jDate: v, gDate: this.jDateFunctions.getGDate(this.persianDate)._toString("YYYY/MM/DD") });
