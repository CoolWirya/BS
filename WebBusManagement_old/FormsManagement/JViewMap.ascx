        ...
        if (ValidateSubmit() == false) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.d[0] != "0") {
                        if (parseInt(data.d[0].toString()) > 0) {
                            if (parseInt(data.d[0].toString()) <= 600) {
                                $("#NumberOfTransactionCount").html("<br />" + "اتوبوس : " + cmbBus.val() + "<div style='clear:both;height:3px'></div>"
                                    + "تعداد تراکنش های دریافت شده : " + data.d[0].toString() + "<div style='clear:both;height:3px'></div>"
                                    + "کد پرسنلی راننده : " + data.d[2].toString() + "<div style='clear:both;height:3px'></div>"
                                    + "نام راننده : " + data.d[3].toString() + "<div style='clear:both;height:3px'></div>"
                                    + "منطقه : " + data.d[4].toString() + "<div style='clear:both;height:3px'></div>"
                                    + "خط : " + data.d[5].toString());
                                if (RunKey == "Path") {
                                    SetMarkers(data.d[1], "True", data.d[6]);
                                }
                                else if (RunKey == "Simulation") {
                                    var MarkerCounter = 0;
                                    var LineColorCounter = 0;
                                    var SimulationMarkers = data.d[1];
                                    var PrevMarker = new Array();
                                    var SimulationLineColor = data.d[6];
                                    ...
                                    DifferenceBetweenTime = data.d[7];
                                    var TimerCounter = 0;
                                    TimerPause = false;
                                ...
                                $("#NumberOfPointSimulation").html("");
                                alert("لطفا بازه ی گزارش خود را کوتاهتر کنید");
                            }
                        }
                        else {
                            $("#NumberOfTransactionCount").html("");
