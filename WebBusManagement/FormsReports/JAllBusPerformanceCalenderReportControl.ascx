<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="JAllBusPerformanceCalenderReportControl.ascx.cs" Inherits="WebBusManagement.FormsReports.JAllBusPerformanceCalenderReportControl" %>
...
<div class="BigTitle">گزارش تقویم کارکرد معین</div>
...
<asp:Panel ID="pnlYearMonth" runat="server">
    <table style="width: 500px">
        ...
        <tr class="Table_RowB">
            <td style="width: 150px">وضعیت تراکنش ها:</td>
            <td>
                <telerik:RadComboBox runat="server" ID="rdTransactionType" Width="100%" Filter="Contains">
                    <Items>
                        <telerik:RadComboBoxItem Value="0" Text="0 - همه" Selected="true" />
                        <telerik:RadComboBoxItem Value="1" Text="1 - بدون تراکنش" />
                        <telerik:RadComboBoxItem Value="2" Text="2 - تایید نشده" />
                        <telerik:RadComboBoxItem Value="3" Text="3 - پرداخت نشده" />
                        <telerik:RadComboBoxItem Value="4" Text="4 - بدون پرینت" />
                        <telerik:RadComboBoxItem Value="5" Text="5 - کامل" />
                        <telerik:RadComboBoxItem Value="6" Text="6 - تایید شده با پرینت" />
                        <telerik:RadComboBoxItem Value="7" Text="7 - برابر با پرینت" />
                        <telerik:RadComboBoxItem Value="8" Text="8 - بیشتر از پرینت" />
                        <telerik:RadComboBoxItem Value="9" Text="9 - کمتر از پرینت" />
                        <telerik:RadComboBoxItem Value="10" Text="10 - عدم ارسال درب ها" />
                    </Items>
                </telerik:RadComboBox>
            </td>
        </tr>
        <tr class="Table_RowB">
            <td style="width: 150px">نمایش بر اساس عدد:</td>
            <td>
                <telerik:RadComboBox runat="server" ID="rdNumberForShow" Width="100%" Filter="Contains">
                    <Items>
                        <telerik:RadComboBoxItem Value="1" Text="1 - پرینت الکترونیک" />
                        <telerik:RadComboBoxItem Value="2" Text="2 - کارکرد" Selected="true"/>
                        <telerik:RadComboBoxItem Value="3" Text="3 - پرداخت شده" />
                    </Items>
                </telerik:RadComboBox>
            </td>
        </tr>
...
<div style="clear: both; height: 15px;"></div>
<telerik:RadButton runat="server" ID="btnGetExcelFile" Text="دریافت فایل اکسل" AutoPostBack="true" Width="100px" Height="35px" OnClick="btnGetExcelFile_Click" />
<div style="clear: both; height: 15px;"></div>
...
<div style="width: 980px; height: auto; margin-right: auto; margin-left: auto">
    <div style="float: right; width: 140px; height: 20px; background-color: #9b9797; color: white; text-align: center">
        1 - برکناری از طرف بازرسان
    </div>
    <div style="float: right; width: 140px; height: 20px; background-color: #9b9797; color: white; text-align: center">
        2 - بازسازی اتوبوس
    </div>
...
<div style="clear: both; height: 5px"></div>
<div style="width: 1530px; height: auto; margin-right: auto; margin-left: auto" id="HtmlMainDiv">
    <%=StrCalender %>
</div>
...
<script type="text/javascript">
    ...
    function SetParam(DateH, BusNumberH, IsSentH) {
      ...
      //  $("#dialog-confirm").attr('title', 'اتوبوس ' + BusNumberH + ' تاریخ ' + DateH);
        $("#DateH").val(DateH);
        $("#BusNumberH").val(BusNumberH);
        $("#IsSentH").val(IsSentH);
        //  $(function () {
        $("#dialog-confirm").attr('title', 'اتوبوس ' + $("#BusNumberH").val().toString() + ' تاریخ ' + $("#DateH").val().toString()).dialog({
            resizable: false,
            height: 140,
            modal: true,
            buttons: {
            ...
            open: function () {
                $(this).dialog("option", "title", 'اتوبوس ' + $("#BusNumberH").val().toString() + ' تاریخ ' + $("#DateH").val().toString());
            }
        });
        // });
    }
    ...
    function GetReport() {
        ...
        paramStr[5] = $("#<%=dteControlFrom.ClientID %>").val();
        paramStr[6] = $("#<%=dteControlTo.ClientID %>").val();
        paramStr[7] = $("#<%=rdTransactionType.ClientID %>").val().toString().split('-')[0];
        ...
        var ShowForNumber = parseInt($("#<%=rdNumberForShow.ClientID %>").val().toString().split('-')[0]);
        ...
        ResultAjaxRequet = $.ajax({
            ...
            success: function (data) {
                ...
                if (parseInt($("#<%=cmbMount.ClientID %>").val().toString().split('-')[0]) <= 6)
                { NumOfDay = 31; }
                else if (parseInt($("#<%=cmbMount.ClientID %>").val().toString().split('-')[0]) > 6 & parseInt($("#<%=cmbMount.ClientID %>").val().toString().split('-')[0]) <= 11)
                { NumOfDay = 30; }
                else
                { NumOfDay = 29; }
