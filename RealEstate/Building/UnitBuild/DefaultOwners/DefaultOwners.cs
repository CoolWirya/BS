...
namespace RealEstate
{
    ...
    public class JDefaultOwner :JSystem
    {
        ...
        public bool GetData(int pCode)
        {
            ...
            try
            {
                DB.setQuery("SELECT * FROM " + JRETableNames.DefaultOwners + " WHERE Code=" + pCode.ToString());
                DB.Query_DataReader();
        ...
        public static string Query = " SELECT  " + JRETableNames.DefaultOwners + ".*, clsAllPerson.Name  FROM  " +
            JRETableNames.DefaultOwners + " inner join clsAllPerson  on clsAllPerson.Code = " +
            JRETableNames.DefaultOwners + ".PCode ";
        ...
        public static DataTable GetDataTable()
        {
            ...
            try
            {
                db.setQuery(Query + " Where " +
                    JPermission.getObjectSql("RealEstate.JDefaultOwner.JDefaultOwner", JRETableNames.DefaultOwners + ".Code"));
                return db.Query_DataTable();
            }
            catch (Exception ex)
            {
        ...
        {
            if (DefaultOwners.Select(" PCode = " + pCode.ToString()).Length > 0)
            {
                return true;
            }
            else
