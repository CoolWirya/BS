...
namespace BusManagment.TransactionPublic
{
    ...
    public class JTransactions
    {
            ...
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    try
                    {
                        ...
                        Tuple<JTransactionHeader, byte[]> result = Process(row["SendLog"] as byte[]);
                        bool isSuccess = true;
                                ...
                                foreach (var AH in result.Item1.AVLHeaders)
                                {
                                    ...
                                    if (AH.AVLs != null && AH.AVLs.Count() > 0)
                                    {
                                        ...
                                        foreach (var AVL in AH.AVLs)
                                        {
                                            ...
                                            if (isSuccess) isSuccess = BusManagment.AVL.JAVLTransactionsPublic.AddAVL(AH, AVL, recordNumber, result.Item1.IMEI, result.Item1.busSerial, result.Item1.VERSION, DB);
                                            if (isSuccess == false)
                                            {
                        ...
                        if (isSuccess)
                        {
                            DBPubic.setQuery("Update SendLogs set Saved = 1,ErrorCount=251, EndProcess=GETDATE(), GSMSerialFound=1 where SendLogId = " + recordNumber.ToString());
                            DBPubic.Query_Execute();
                        }
                        else
                        {
                            DBPubic.setQuery("Update SendLogs set ErrorCount=ErrorCount+1, EndProcess=GETDATE(),GSMSerialFound=1 where SendLogId = " + recordNumber.ToString());
                            DBPubic.Query_Execute();
                        }
                        //else
                        //db.Rollback("avl");
        ...
        public void CheckDataTicket(ClassLibrary.JDataBase db)
        {
            System.Data.SqlClient.SqlConnectionStringBuilder C = new System.Data.SqlClient.SqlConnectionStringBuilder(@"
                Data Source=95.80.158.130;
                Initial Catalog=BusAvlSendLogs;
            ...
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    try
                    {
                        ...
                        if (result.Item1.ErrorNum == 200)
                        {
                            DBPubic.setQuery("Update SendLogs set ErrorCount = 250,Saved = 0 , EndProcess=GETDATE(), GSMSerialFound=1 where SendLogId = " + recordNumber.ToString());
                            DBPubic.Query_Execute();
                            continue;
                        }
                        ...
                        if (isSuccess)
                        {
                            DBPubic.setQuery("Update SendLogs set Saved = 1,ErrorCount=100,EndProcess=GETDATE(), GSMSerialFound=1 where SendLogId = " + recordNumber.ToString());
                            DBPubic.Query_Execute();
                        }
                        else
                        {
        ...
        public void CheckDataOfflineTicketPublic()
        {
            ...
            try
            {
                Archivedb.setQuery("SELECT [Code] ,cast([Contents] as varchar(max))Contents FROM [ArchiveContent]  where Status=1 and FileExtension like N'.ini'");
                DataTable DT = Archivedb.Query_DataTable();
                int Code = 0;
                string Content = "";
                ...
                foreach (DataRow DR in DT.Rows)
                {
                    ...
                    string DriverRFID = ContentDetailes[2].ToString().Replace("\r\n", "");
                    string LineNumber = Convert.ToInt32(ContentDetailes[7].ToString().Substring(2, 4)).ToString();
                    string TicketPrice = (Convert.ToInt32(ContentDetailes[8].ToString().Substring(ContentDetailes[8].ToString().Length - 4, 4)) / 10).ToString();
                    ...
                    for (int i = 21; i < TicketList.Length; i++)
                    {
                        ...
                        {
                            DateTime EventDate = new DateTime(Convert.ToInt32(TicketDetailes[3].ToString().Substring(0, 2)) + 2000, Convert.ToInt32(TicketDetailes[3].ToString().Substring(2, 2)),
                                Convert.ToInt32(TicketDetailes[3].ToString().Substring(4, 2)), Convert.ToInt32(TicketDetailes[2].ToString().Substring(0, 2)),
                                Convert.ToInt32(TicketDetailes[2].ToString().Substring(2, 2)), Convert.ToInt32(TicketDetailes[2].ToString().Substring(4, 2)));
                            FinalQueryToRun.Append(@"INSERT INTO [dbo].[AUTTicketTransaction]
                                             ...
                                             VALUES
                                                   ...
                                                   ,0
                                                   ,cast(0x" + TicketDetailes[1].ToString().Replace("\r\n","") + @" as bigint)
                                                   ,0
                                                   ," + TicketPrice + @"
                                                   ,0
                                                   ," + ReaderId + @"
                        ...
                        if (i % 200 == 0)
                        {
                            ...
                            while (JDataBase.dbsOpen.Count > 50) System.Threading.Thread.Sleep(100);
                            JDataBase DbInsert = new JDataBase();
                            DbInsert.setQuery(FinalQueryToRun.ToString());
                            if (DbInsert.Query_Execute(true) > 0)
                            {
