...
namespace BusManagment.AVL
{
    ...
    {
        public static bool AddAVL(Transaction.JTransactionAVLHeader AH, Transaction.JTransactionAVL AVL, Int64 recordNumber, string Header_IMEI, float Header_BusSerial, byte[] Header_Version, ClassLibrary.JDataBase db)
        {
            ...
            db.setQuery(@"EXEC SP_InsertAUTAvlTransaction_Zarrin_Save_Just_Point
	                    ...
	                    @TransactionID,
                        @Dir");
            db.AddParams("Longitude", Transaction.JTransactions.ConvertNMEAToDecimal(AVL.Lon));
            db.AddParams("Latitude", Transaction.JTransactions.ConvertNMEAToDecimal(AVL.Lat));
