...
namespace BusManagment.Driver
{
    ...
    public class JDriverseLogs
    {
        ...
        public static string GetWebDriverPerformanceReportQueryWithMultiDriver(int[] DriverCode, DateTime? StartEventDate = null, DateTime? EndEventDate = null, int ZoneCode = 0, int LineNumber = 0, int BusNumebr = 0, int CardType = -1, int BusOwner = 0)
        {
            ...
            Query = @"select top 100 percent DP.Code,DP.DriverPersonCode as PesonelCode,ALLP.Name as DriverName,DP.[Date],AF.Name as FleetName,AZ.Name as ZoneName,
                            DP.LineNumber,AB.BUSNumber,DP.CardType,DP.TCount as TransactionCount,DP.Price * 10 as InCome
							...
							left join [dbo].[clsAllPerson] ALLP on DP.DriverPersonCode = ALLP.Code " + WhereStr + PermitionSql + FinalCardType;

            return Query;
        }

        public static string GetWebDriverPerformanceReportQuery(int DriverCode = 0, int ZoneCode = 0, int LineNumber = 0, int BusNumebr = 0, int DayType = -1, DateTime? StartEventDate = null, DateTime? EndEventDate = null, int OwnerCode = 0, bool CalcService = false, int CardType = -1, int FleetCode = 0)
        {
            ...
            string Query = "", WhereStr = " where Dp.TCount > 0 and DP.Price > 0 and DP.TicketPrice > 0 and DP.Error = 0 ";
            DateTime NullDatetime = new DateTime(0001, 1, 1, 12, 00, 00);
            ...
            Query = @"select top 100 percent max(DP.Code) as Code,cap.Name as OwnerName,DP.DriverPersonCode as PersonelCode,ALLP.Name as DriverName,DP.DriverCardSerial
                            ...
                            left join AUTBusOwner abo on DP.BusCode = abo.BusCode
