        ...
        public DataTable GetWebOwners(int pBusCode)
        {
            ...
            try
            {
                DB.setQuery(@"Select
                                    ...
                                    (Select Fa_Date FROM StaticDates Where En_Date =  AUTBusOwner.StartDate) as N'تاریخ شروع',
                                    (Select Fa_Date FROM StaticDates Where En_Date =  AUTBusOwner.EndDate) as N'تاریخ پایان',
        ...
        public static DataTable GetDataTable(int pBusCode)
        {
            ...
            try
            {
                DB.setQuery(@"SELECT 
		                ...
		                ,(Select Fa_Date FROM StaticDates Where En_Date =  AUTBusOwner.StartDate) StartDate
		                ,(Select Fa_Date FROM StaticDates Where En_Date =  AUTBusOwner.EndDate) EndDate
		                FROM dbo.AUTBusOwner  
		                INNER JOIN dbo.AUTBus  ON AUTBusOwner.BusCode = AUTBus.Code
		                INNER JOIN dbo.clsAllPerson  ON clsAllPerson.Code = AUTBusOwner.CodePerson " +
                          " WHERE AUTBusOwner.BusCode=" + pBusCode.ToString());
