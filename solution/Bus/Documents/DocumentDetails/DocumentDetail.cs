            ...
            try
            {
                ...
                if (DocumentCode > 0)
                {
                    query = @"select TOP 100 PERCENT " + DocumentCode + @" DocumentCode,a.TafziliCode1 OwnerPCode,(select Name From clsAllPerson where Code = TafziliCode1)OwnerName,
                                ...
                                select TafziliCode1,cast(sum(BesPrice) as float)TottalPrice,cast(sum(BesPrice) as float)PaymentPrice from FinDocumentDetails where DocCode in(" + DocumentCode + @") and TafziliCode1 <> 20000001
