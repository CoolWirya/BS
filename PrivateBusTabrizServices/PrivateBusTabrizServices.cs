...

    {
        ...
        private void AVLProcess()
        {
            ...
            try
            {
                ...
                {
                    Thread.Sleep(50);
                    BusManagment.Transaction.JTransactions.IsProcess++;
                    ...
                    if (BusManagment.Transaction.JTransactions.IsProcess > 100)
                    {
                        BusManagment.Transaction.JTransactions.SocketCheckDataAVLThread = 0;
                    }
        ...
        private void AVLProcessTeltonika()
        {
            ...
            try
            {
                ...
                {
                    Thread.Sleep(50);
                    BusManagment.Transaction.JTransactions.IsProcessTeltonika++;
                    ...
                    if (BusManagment.Transaction.JTransactions.IsProcessTeltonika > 100)
                        BusManagment.Transaction.JTransactions.SocketCheckDataTeltonikaAVLThread = 0;
                }
                ...
                try
                {
                    ...
                    else
                    {
                        Int64 Index = ClassLibrary.Socket.JSocketManager.GetData(client, bytes, "BusManagment.Transaction.JTransactionAVL.GetClassNameBusNumber"
                            , 0, BusManagment.Transaction.SocketDataState.Bus_InProcess.GetHashCode()
                            );
                        if (Index > 0)
                        {
                            BspTcpServer.SendData(client, "OK");
                            BusManagment.Transaction.JTransactionAVL.GetClassNameBusNumber(bytes, Index, false);
                        }
                        else
                        {
                            BspTcpServer.SendData(client, "FALSE");
        ...
        private void AnswerRtpisSMSFunction()
        {
            while (_AnswerRtpisSMS)
            {
                try
                {
                    ...
                    request.Method = "POST";
                    string postData = "op=inboxlist&uname=aryafajrkimia&pass=@Fk4994";
                    ...
                    if (responseFromServer.Length > 10)
                    {
                        ...
                        foreach (string message in messages)
                        {
                            ...
                            ClassLibrary.JSMSGet SG = new ClassLibrary.JSMSGet();
                            SG.Mobile = From;
                            SG.PersonCode = 0;
                            SG.Text = Text;
                            SG.ReadDate = DateTime.Parse(Time);
                            SG.Insert(db);
                            db.Dispose();
