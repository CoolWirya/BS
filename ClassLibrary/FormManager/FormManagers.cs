    ...
    public class FormManagers
    {
        ...
        public void GetFromManager(int FormCode)
        {
            ...
            try
            {
                ...
                string Basesql = (new JForms(FormCode)).BaseSQL;
                string Wheresql = (new JForms(FormCode)).WhereSQL;
                //if (sql == "") sql = "Select * from [Propperty_ClassName_ClassLibrary.FormManagers_1000018]";
                ...
                query = "Select " + query + ",a.* From " + TableName + " f inner join ( " +  Basesql + ") as a ON a."+
                    Wheresql.Replace("@ObjectCode", "(select ObjectCode from FormObjects WHERE Code = f.ObjectCode)");
