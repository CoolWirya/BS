...
namespace ClassLibrary
{
	...
	public class JPermission : JSystem
	{
        ...
        public static bool CheckPermission(string pClassName, int pObjectCode, int pPostCode, bool pShowMessgae, string pMessage)
		{
            ...
            {
                string query;
                if(PermissionDataTable==null || (PermissionDataTable != null && PermissionDataTable.PrimaryKey.Length == 0))
                {
                ...
                if (!HasPermission)
                {
                    ...
                    if (PermissionDataTableGroup == null || (PermissionDataTableGroup != null && PermissionDataTableGroup.PrimaryKey.Length == 0))
                    {
                        query =
            ...
            {
                if (WebClassLibrary.SessionManager.Current.Session[PermissionTableName + "_DataTable"] == null)
                {
                ...
                if (!HasPermission)
                {
                    ...
                    //H.Date = DateTime.Now;
                    //H.Save();
                    if (WebClassLibrary.SessionManager.Current.Session[PermissionTableName + "_Groups_DataTable"] == null)
                    {
                        string query =
                            @"select Class_Name,ObjectCode from PermissionGroupUsers pgu
                            inner join PermissionGroup pg on pgu.GroupCode = pg.GroupCode
