    ...
    public partial class AvlService : ServiceBase
    {
        ...
        private void GeofenceCheck()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        try
                        {
                            ...
                            coordinate.Angle = float.Parse(dr["lastAngle"].ToString());
                ...
                {
                    System.IO.File.AppendAllText(@"C:\GeofenceLog.txt", er.Message + " [" + DateTime.Now + "](StackTrace" + er.StackTrace + ")" + Environment.NewLine + Environment.NewLine);
                }
                finally
                {
                }
        ...
        private void SpeedController()
        {
            ...
            while (true)
            {
                ...
                {
                    System.IO.File.AppendAllText(@"C:\SpeedCheckLog.txt", er.Message + " [" + DateTime.Now + "](Line" + er.StackTrace + ")" + Environment.NewLine);
                }
                finally
                {
                    DB.Dispose();
                    ...
                    if (dt != null)
                    {
                        ...
                        if (drs != null && drs.Count > 0)
                            foreach (System.Data.DataRow dr in drs)
                            {
                                ...
                                {
                                    System.IO.File.AppendAllText(@"C:\ObjectListUpdateLog.txt", er.Message + " [" + DateTime.Now + "](Line" + er.StackTrace + ")" + Environment.NewLine);
                                }
                                finally
                                {
                                    DB.Dispose();
        ...
        private void RunServices()
        {
            ...
            string[] lines = System.IO.File.ReadAllLines(@"C:\services.txt");
            if (lines[0] == "t")
                StartSocketListner();
        ...
        private void ExtractorRunner(object obj)
        {
            ...
            while (true)
            {
                ...
                {
                    System.IO.File.AppendAllText(@"C:\ExtractorLog.txt", er.Message + " [" + DateTime.Now + "][" + methodname + "](Line" + er.StackTrace + ")" + Environment.NewLine);
                }
            }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        while (DB.DataReader.Read())
                        {
                            if (!threadNames.Contains(DB.DataReader["TableName"].ToString()))
                            {
                                System.Threading.Thread t = new Thread(new ParameterizedThreadStart(ExtractorRunner));
                                t.Name = DB.DataReader["TableName"].ToString();

                                if (!threads.Contains(t))
                                {
                                    t.Start(new AVL.GPSData.JDataKeeper(DB.DataReader));
                                    threadNames.Add(DB.DataReader["TableName"].ToString());
                                    threads.Add(t);
                                }
                                else
                                    t = null;
                    ...
                    {
                        DB.Dispose();
                        System.IO.File.AppendAllText(@"C:\DataKeeperLog.txt", er.Message + " [" + DateTime.Now + "](stacktrace:" + er.StackTrace + ")" + Environment.NewLine);
                    }
                    finally
                    {
                        DB.Dispose();
        ...
        public void EmailSend()
        {
            ...
            while (true)
            {
                ...
                {
                    System.IO.File.AppendAllText(@"C:\EmailLog.txt", er.Message + " [" + DateTime.Now + "](Line" + er.StackTrace + ")" + Environment.NewLine);
                }
            }
        }
        public void SendSMSProcess()
        {
            ...
            while (_SMSSender)
            {
                ...
                catch (Exception er)
                {
                    ...
                    System.IO.File.AppendAllText(@"C:\SMSLog.txt", er.Message + " [" + DateTime.Now + "](Line" + er.StackTrace + ")" + Environment.NewLine);
                }
                finally
                {
                    Db.Dispose();
        ...
        private void BspTcpServer_OnReceiveData(object sender, System.Net.Sockets.TcpClient client, byte[] bytes, string data)
        {
            ...
            if (bytes[0] == (byte)150)
            {
                if (bytes[1] == (byte)1)
                {
                    ...
                    {
                        System.IO.File.AppendAllText(@"C:\SocketLog.txt", er.Message + " [" + DateTime.Now + "](Line" + er.StackTrace + ")" + Environment.NewLine);
                    }
                    finally
                    {
                        DB.Dispose();
        ...
        private void BspTcpServer_OnError(object sender, System.Net.Sockets.TcpClient client, Exception exception)
        {
            ...
            System.IO.File.AppendAllText(@"C:\SocketLog.txt", exception.Message + " [" + DateTime.Now + "](Line" + exception.StackTrace + ")" + Environment.NewLine);
