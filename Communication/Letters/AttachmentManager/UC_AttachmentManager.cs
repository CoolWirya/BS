...
namespace Communication
{
    public partial class JUC_AttachmentManager : UserControl
    {
        ...
        /// <summary>
        /// دست آوریدن پسوند فایل از روی نام فایل
        /// </summary>
        /// <param name="pfileName">نام فایل</param>
        /// <returns>پسوند</returns>
        private string GetExtension(string pfileName)
        {
        ...
        private void Word_BeforUpdateSave(object sender, EventArgs e)
        {
            ...
            {
                if (WordUpdateDocument[i] != null && WordUpdateDocument[i].Code == ((Communication.JCOfficeWord)(sender)).Code)
                {
        ...
        /// <summary>
        /// بر می گرداند datatable این تابع مشخصات ضمائم را بصورت  
        /// </summary>
        /// <returns>DataTable</returns>
        public DataTable GetData()
        {
            DataTable dt = new DataTable();
            dt = (new JCLetterAttachment()).GetDataTablePatern();
            for (int i = 0; i < lstvFiles.Items.Count; i++)
            {
                DataRow dr = dt.NewRow();
                Attachment attach = ((Attachment)lstvFiles.Items[i].Tag);
                dr["Code"] = attach.Code;
                dr["file_name"] = attach.FileName;
                if (attach.FileTitle == null)
                {
                    dr["file_title"] = lstvFiles.Items[i].Text;
                }
                else
                {
                    dr["file_title"] = attach.FileTitle;
                }
                dr["file_type"] = attach.FileType;
                dr["create_date_time"] = (new JDataBase().GetCurrentDateTime());
                dr["file_content"] = attach.FileContent;                
                dr["file_text"] = attach.FileText;
                dr["is_main_file"] = attach.Is_Main_File;
                dr["other_file_type"] = attach.other_file_type;
                dr["previous_version_code"] = attach.previous_version_code;                
                dr["is_new"] = attach.Is_New;
                dt.Rows.Add(dr);
            }
            return dt;
        }
        ...
        /// <summary>
        /// تنطیم آخرین اطلاعات کنترل بر اساس کد نامه وارد شده
        /// </summary>
        /// <param name="pLetterCode"></param>
        public void SetDataLast(int pLetterCode)
        {
            int codeWord = 0;
            Array.Resize(ref WordUpdateDocument, 0);
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        if (codeWord!=0)
                            lstvFiles.Items[codeWord-1].Remove();
                        ltvItem.Group = lstvFiles.Groups["lvgWord"];
                        if (Attach.Is_Main_File)
                            ltvItem.ImageIndex = Icons.Word_Is_Main_File;
                        else
                    ...
                    if ((lstvFiles.Items.Count > 0)&&(Attach.FileType == ClassLibrary.Domains.JCommunication.JFileType.word))
                        codeWord = lstvFiles.Items.Count;
                }
            }
            catch (Exception ex)
            {
        ...
        private void lstvFiles_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            try
            {
                string strExtension = GetExtension(((Attachment)lstvFiles.SelectedItems[0].Tag).FileName);
        ...
        {
            string strExtension  = GetExtension(((Attachment)e.Item.Tag).FileName);
