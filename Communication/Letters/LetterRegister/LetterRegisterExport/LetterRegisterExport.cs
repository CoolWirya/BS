...
namespace Communication
{
    ...
    /// </summary>
    public class JCLetterRegisterExport : JCLetterRegister
    {
        ...
        /// سازنده
        /// </summary>
        public JCLetterRegisterExport()
            : base(0)
        {
        }

        public JCLetterRegisterExport(int pCode)
            : base(pCode)
        {
        }
        ...
        {
            if (JPermission.CheckPermission("Communication.JCLetterRegisterExport.Insert"))
            {
                ...
                try
                {
                    DB.beginTransaction("Save Export Letter");
        ...
        public bool Update(int pLetterCode, DataTable pDTCopyes, ArchivedDocuments.JArchiveList pDTAttachments, OfficeWord.WinWordControl pLetter, DataTable pDTRelatedLetter)
        {
            JDataBase DB = new JDataBase();
            try
            {
                DB.beginTransaction("LetterRegister");
                if (JPermission.CheckPermission("Communication.JCLetterRegisterExport.Update"))
                {
                    Code = pLetterCode;
                    //----------- اطلاعات کاربر دریافت کننده -----------------------------------------------------------------------
                    //Employment.JEOrganizationChart EOC = new Employment.JEOrganizationChart(sender_post_code);
                    //sender_full_title = EOC.full_title;
                    //sender_code = EOC.user_code;


                    ClassLibrary.JAllPerson Pers = new JAllPerson(receiver_external_code);
                    receiver_full_title = Pers.Name;
                    receiver_external_code = Pers.Code;
                    ...
                    if (RegSignatureLetter && letter_status == ClassLibrary.Domains.JCommunication.JLetterStatus.Accept && RegisterInSecretariat(DB))
                    {
                    }
                    else
                    {
                    ...
                    if (letter_status == ClassLibrary.Domains.JCommunication.JLetterStatus.Accept && RegSecretariats(DB))
                    {
                        RegInSevretariat = true;
                    }
                    else
        ...
        {
            JfrmLetterRegister jLRImport = new JfrmLetterRegister(ClassLibrary.Domains.JCommunication.JLetterType.Export, JFormState.Insert, pLetter_Code, Current_Refer_Code);
            if (pLetter_Code == 0)
        ...
        {
            JfrmSearchLetters jLRImport = new JfrmSearchLetters(ClassLibrary.Domains.JCommunication.JLetterType.Export,0);
            jLRImport.ShowDialog();
        }

        public JNode GetNode(DataRow pRow)
        {
            JNode node = new JNode((int)pRow["Code"], "Communication.JCLetterRegisterExport");
            node.Icone = JImageIndex.testimonial.GetHashCode();
            node.Name = pRow["Sender_Full_Title"].ToString();

            Nodes.hidColumns = @"Sender_Name,Sender_Post_Name,Sender_Title_Job,Receiver_Name,Receiver_External_Name
,letter_type,sender_post_code,sender_code,sender_external_code,receiver_post_code,receiver_code,register_post_code,register_user_code,send_type,receiver_type,subject_code,sender_external_code,receiver_external_code";
            ...
            /// اکشن ویرایش
            JAction editAction = new JAction("Edit...", "Communication.JCLetterRegisterExport.ShowDialog", new object[] { node.Code }, null);
            ...
            /// اکشن جدید
            JAction newAction = new JAction("New...", "Communication.JCLetterRegisterExport.ShowDialog", null, null);
            /// اکشن آرشیو
    ...
    ///  کلاس ثبت نامه وارده
    /// </summary>
    public class JCLetterRegisterExports : JSystem
    {
        // سازنده های کلاس
        #region Constructors
        /// <summary>
        /// سازنده
        /// </summary>
        public JCLetterRegisterExports()
        {
        }
        #endregion Constructors
        ...
        {
            Nodes.ObjectBase = new JAction("GetExportLetters", "Communication.JCLetterRegisterExport.GetNode");
            Nodes.DataTable = GetDataTable();

            JAction newAction = new JAction("New...", "Communication.JCLetterRegisterExport.ShowDialog", new object[] { 0 }, null);
            ...
            JAction SearchAction = new JAction("Search...", "Communication.JCLetterRegisterExport.SearchDialog", null, null);
...
summary 
                from " + JTableNamesLetters.Letters + " Where  letter_type=" +
                    ClassLibrary.Domains.JCommunication.JLetterType.Export + " And register_user_code=" + JMainFrame.CurrentUserCode +
