...
namespace Communication
{
    ...
    public partial class JfrmLetterRegisterExport : Globals.JBaseForm
    {
        ...
        public void SetForm(int pLetterCode)
        {
            ...
            JCLetterRegisterExport LRI = new JCLetterRegisterExport(pLetterCode);
            //------------------رونوشت ---------------
            ...
            {
                JOrganization OrganTemp = new JOrganization(LRI.sender_external_code);
                cdbSender.SetValue(OrganTemp.Access_Code);
            }
            else
                cdbSender.Text = LRI.sender_full_title;
            cdbReceiver.SetValue((new ClassLibrary.JAllPerson(LRI.receiver_post_code)).Code);
        ...
        private bool WorkingMinuteExport(JCWorkLetterType pAction)
        {
            ...
            else if (State == JFormState.Update)
            {
                ...
                if (LRI.Update(_LetterCode, _dtCopyes, jArchiveList1, winWordControl1, _dtRelatedLetter))
                {
        ...
        private void Data(JCLetterRegister LRI)
        {
            ...
            //----------------- وضعیت نامه ----------------------------------------
            LRI.letter_type = ClassLibrary.Domains.JCommunication.JLetterType.Export;
            //------------------ ضمائم --------------------------------------------
        ...
        {
            JCLetterRegisterExport LRI = new JCLetterRegisterExport(pCode);
            DataRow dr;
            DataTable dt = new DataTable();
        ...
        private void txtLetterNo_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    JCLetterRegisterExport LRI = new JCLetterRegisterExport(Convert.ToInt32(txtLetterNo.Text));
                    if (LRI.Code > 0)
        ...
        private void Filter_Receiver()
        {
            ...
            if (cdbSender.SelectedItem != null)
                UC_LetterCopy1._SetComboBoxs(Convert.ToInt32(cdbSender.SelectedItem["Code"]), ClassLibrary.Domains.JCommunication.JLetterType.Export);
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            {
                JfrmLetterRegisterExport tmp = new JfrmLetterRegisterExport(ClassLibrary.Domains.JCommunication.JLetterType.Export, JFormState.ReadOnly, Convert.ToInt32(uC_GridLetter.SelectedRow.Cells["Code"].Value.ToString()), _Refer_Code);
                tmp.ShowDialog();
            }
            else
                JMessages.Message("Please Selected Minute ", "", JMessageType.Information);
