...
namespace FastReport.Preview
{
  ...
  public class PreparedPages : Component
  {
    ...
    {
      ReportPage page = FPreparedPages[index].GetPage(index, FPreparedPages.Count);
    ...
    internal void ClearFirstPass(int pageNo, int position)
    {
      Bookmarks.ClearFirstPass();
      Outline.Clear();
      
      // clear all pages after specified pageNo
      while (pageNo < Count - 1)
      {
        RemovePage(Count - 1);
      }

      // if position is at begin, clear all pages
      if (pageNo == 0 && position == 0)
        RemovePage(0);

      // delete objects on the pageNo
      if (pageNo >= 0 && pageNo < Count)
        FPreparedPages[pageNo].CutObjects(position);
    }
