...
namespace FastReport
{
  ...
  [Designer(typeof(ReportComponentDesigner))]
  [DesignerSerializer(typeof(ReportCodeDomSerializer), typeof(CodeDomSerializer))]
  public class Report : Base, IParent, ISupportInitialize
  {
    ...
    private void SavePreviewPicture()
    {
      ...
      ReportInfo.Picture = new Bitmap((int)Math.Round(pageWidth * 0.1f), (int)Math.Round(pageHeight * 0.1f));

      using (Graphics g = Graphics.FromImage(ReportInfo.Picture))
      {
        FRPaintEventArgs args = new FRPaintEventArgs(g, 0.1f, 0.1f, GraphicCache);
        page.Draw(args);
      }
    }
    ...
    /// Otherwise returns <b>null</b>.</returns>
    /// <remarks>
    /// Do not call this method directly. Use the <b>GetColumnValueNullable(string complexName)</b> instead.
    /// </remarks>
    ...
    public override void Serialize(FRWriter writer)
    {
      ...
      if (ScriptLanguage != c.ScriptLanguage)
    ...
    {
#if Basic
      throw new Exception("The FastReport.Net Basic edition does not allow run-time report designer.");
#else      
      if (FDesigner != null)
        return false;
    ...
    /// <param name="data">The application data.</param>
    /// <param name="name">The name of the data.</param>
    public void RegisterDataAsp(System.Web.UI.IDataSource data, string name)
    {
      FAspDataName = name;
      System.Web.UI.DataSourceView view = data.GetView("");
      if (view != null)
        view.Select(System.Web.UI.DataSourceSelectArguments.Empty, new System.Web.UI.DataSourceViewSelectCallback(RegisterDataAsp));
    }

    private string FAspDataName;
    private void RegisterDataAsp(IEnumerable data)
    {
      RegisterDataAsp(data, FAspDataName);
    }

    internal void RegisterDataAsp(IEnumerable data, string name)
    {
      DataComponentBase existingDs = Dictionary.FindDataComponent(name);
      if (existingDs is ViewDataSource && data is DataView)
      {
        // compatibility with old FR versions (earlier than 1.1.45)
        Dictionary.RegisterDataView(data as DataView, name, true);
      }
      else
      {
        // in a new versions, always register the business object
        Dictionary.RegisterBusinessObject(data, name, 1, true);
      }
    }
    ...
    /// <summary>
    /// Prints the report. Report should be prepared using the <see cref="Prepare()"/> method.
    /// </summary>
