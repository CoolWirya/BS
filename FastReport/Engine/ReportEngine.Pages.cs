...
namespace FastReport.Engine
{
  public partial class ReportEngine
  {
    ...
    private DataBand FindDeepmostDataBand(BandCollection bands)
    {
      if (bands.Count == 0)
        return null;
      ...
      if (band is DataBand)
        result = band as DataBand;
      else if (band is GroupHeaderBand)
        result = (band as GroupHeaderBand).GroupDataBand;
      
      if (result == null)
        return null;
      DataBand nestedBand = FindDeepmostDataBand(result.Bands);
      return nestedBand == null ? result : nestedBand;
    }
    ...
    private void RunReportPage(ReportPage page)
    {
      ...
      DataBand keepSummaryBand = FindDeepmostDataBand(page.Bands);
