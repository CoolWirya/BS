...
namespace FastReport
{
  ...
  public class TextObject : TextObjectBase, IHasEditor
  {
    ...
    private void Engine_ColumnFinished(object sender)
    {
      ...
      // row is going to print on a new column. Reset the text and recalc the band height.
      FLastText = null;
      Text = FSavedText;
      Visible = true;
      GetData();
    ...
    /// <inheritdoc/>
    public override void ExtractMacros(int pageIndex, int totalPages)
    {
      base.ExtractMacros(pageIndex, totalPages);
      Text = Text.Replace("[TOTALPAGES#]", totalPages.ToString());
      Text = Text.Replace("[PAGE#]", (pageIndex + 1).ToString());
    }
    #endregion
