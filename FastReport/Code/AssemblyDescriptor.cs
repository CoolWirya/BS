...
namespace FastReport.Code
{
  internal class AssemblyDescriptor
  {
    ...
    private string GetFullAssemblyReference(string relativeReference)
    {
      ...
      string dllName =
        string.Compare(relativeReference.Substring(relativeReference.Length - 4), ".dll", true) == 0 ?
        relativeReference.Substring(0, relativeReference.Length - 4) : relativeReference;
    ...
    private void InternalCompile()
    {
      ...
      if (Config.WebMode && Directory.Exists(currentFolder + @"Bin\"))
        currentFolder += @"Bin\";
