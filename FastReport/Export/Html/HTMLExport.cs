    ...
    public class HTMLExport : ExportBase
    {
        ...
        private string HTMLGetImage(ExportIEMObject obj, int PageNumber, int CurrentPage, int ImageNumber)
        {
            if (FPictures)
            {
                ...
                string ImageFileName = Path.GetFileName(FTargetFileName) + "." + PageNumber.ToString() + "." + (ImageNumber + 1).ToString() + "." + format.ToString().ToLower();
        ...
        {           
            ...
            if (!FWebMode)
            {
                ...
                {
                    FSinglePage = true;  // need fix in future
                    FNavigator = false;
                    FPictures = false;                    
                }
                else
