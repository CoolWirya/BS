    ...
    public class JTransactions
    {
        ...
        public void CheckDataTicketOnlineReader()
        {
            ...
            try
            {
                ...
                lock (listLock)
                {
                    DB.setQuery("Select * from AUTOnlineReaderTicket where IsProceced=0 order by Code desc");
                    dt = DB.Query_DataTable();
                }
                foreach (DataRow row in dt.Rows)
                {
        ...
        public void CheckDataOnlineReaderTicket(byte[] Data, int pCode)
        {
            ...
            try
            {
                ...
                Buffer.BlockCopy(Data, 12, bModuleVersion, 0, 3);
                ...
                Buffer.BlockCopy(Data, 15, bReaderVersion, 0, 3);
        ...
        public static byte[] GetOnlineReaderConfig(DataRow row, out UInt16 crc1, out byte[] _data2, out UInt16 crc2)
        {
            ...
            try
            {
                ...
                byte[] datetimeByteArray = BitConverter.GetBytes(JTransactions.ConvertDateTimeToUint(DateTime.Parse(row["DateTime"].ToString())));
        ...
        public void CheckDataSQLiteTicket()
        {
            ...
            try
            {
                ...
                foreach (DataRow DR in DT.Rows)
                {
                    ...
                    ProcessSqliteDataTable(SQLiteDT, ref TicketDT, ref trueCount, ref falseCount);
        ...
        public static void ProcessSqliteDataTable(DataTable SQLiteDT, ref DataTable TicketDT, ref int trueCount ,ref int falseCount)
        {
            if (SQLiteDT != null)
            {
                trueCount = 0;
            ...
            try
            {
                ...
                try
                {
                    if (result != null)
                    {
                        ...
                        isSuccess = BusManagment.AVL.JAVLTransactions.AddAVL(DT, "update ClsSocketClientDataManager set IsProceced=0 where code = " + pRecordNumber);
                        if (isSuccess == false)
                        {
                            {
                                if (!_UpdateSocketCheckDataAVL)
                ...
                try
                {
                    if (resultList != null)
                    {
                        ...
                        isSuccess = BusManagment.AVL.JAVLTransactions.AddAVL(DT, "update ClsSocketClientDataManager set IsProceced=10 where code = " + pRecordNumber);
                        if (isSuccess == false)
                        {
                            {
                                if (!_UpdateSocketCheckDataAVL)
