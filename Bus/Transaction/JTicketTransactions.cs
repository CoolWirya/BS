    ...
    public class JTicketTransactions
    {
        ...
        public static void ReadDataTableFromFile(string[] pFileName, int pCount, bool IsTicket = true)
        {
            try
            {
                ...
                if (IsTicket && DT != null)
                    ...
                    BusManagment.AVL.JAVLTransactions.AddAVL(DT, "");
            }
            catch (Exception ex)
            {
        ...
        public static void ProcessOldFileTicketDataTable(bool IsTicket = true)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                if (System.IO.File.Exists(files[i].FullName) && System.IO.File.GetCreationTime(files[i].FullName) < DateTime.Now.AddMinutes(-1))
                {
        ...
        public static bool AddTicket(ref System.Data.DataTable pDT, string pErrorCommand, bool pCheckPassenger = true)
        {
            ...
            try
            {
                ...
                int pageNum = 1;
                int pageSize = pDT.Rows.Count <= 500 ? pDT.Rows.Count / 4 + 1 : 500;
                while (true)
                {
                    ...
                    if (DR != null && DR.Count() > 0)
                    {
                        ...
                        pDT = DR.Take(pDT.Rows.Count - dtPage.Rows.Count).CopyToDataTable();
